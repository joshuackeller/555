...
leo_predictions_2 <- leo_fit %>% 
  predict(new_data = leo_testing,
          type = 'prob')



test_results <- leo_testing %>% 
  # select(survived)
  bind_cols(leo_predictions, leo_predictions_2)
...
leo_last %>% collect_metrics()
leo_last %>% collect_predictions()